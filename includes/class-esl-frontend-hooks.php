...
class ESL_FrontendHooks
{
    ...
    {
        $this->google_auth = new ESL_GoogleAuth($plugin_name, $version);
    ...
    {
        wp_enqueue_style('esl-login', ESL_PLUGIN_URL . 'assets/css/login.css', [], ESL_VERSION);
    ...
    private function prepareGoogleSignInScripts($context = 'signin', $show_prompt = false)
    {
        ...
        wp_enqueue_script('esl-login', ESL_PLUGIN_URL . 'assets/js/login.js', ['google-gsi'], ESL_VERSION, true);

        $csrf_token = bin2hex(random_bytes(32));
        setcookie('esl_csrf_token', $csrf_token, [
        ...
        wp_localize_script('esl-login', 'esl_login_params', [
            'client_id' => $client_id,
...
?>
        <div class="esl-login-container">
            <div class="esl-login-header">
            ...
            <div class="esl-login-content">
                <a href="<?php echo esc_url($auth_url); ?>" class="esl-google-button" id="google-login-btn">
                    <svg class="esl-google-logo" viewBox="0 0 24 24" fill="none">
...
<?php
    ...
    public function displayLoginErrors($errors)
    {
        ...
        if (isset($_GET['esl_error_key'])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $error_key = sanitize_key($_GET['esl_error_key']);
