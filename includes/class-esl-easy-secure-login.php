...
?>
            <h3><?php esc_html_e('Google Account Information', 'easy-secure-login'); ?></h3>
                ...
                <tr>
                    ...
                    <td>
                        <span class="esl-auth-badge">
                            ...
                            <?php esc_html_e('Google Sign-In', 'easy-secure-login'); ?>
                        </span>
                        <p class="description"><?php esc_html_e('This user authenticates via Google OAuth.', 'easy-secure-login'); ?></p>
                    </td>
                </tr>
            </table>
<?php
    ...
    private function blockSensitiveFiles()
    {
        ...
        foreach ($sensitive_files as $file) {
            if (stripos($request_uri, $file) !== false) {
                status_header(403);
                die(esc_html__('Access Denied', 'easy-secure-login'));
            }
        }
    }
    ...
    public function restrictRestApiAccess($result)
    {
        ...
        return new WP_Error('rest_not_logged_in', esc_html__('You are not currently logged in.', 'easy-secure-login'), ['status' => 401]);
    }
}
